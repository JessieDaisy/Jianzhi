...
(function () {
  ...
  function searchScenes(genre, query) {
    ...
    index.forEach(function (rec) {
      ...
      rec.lines.forEach(function (line) {
        ...
        /* ① 整词连写 ② 两个词挨着 ③ 同句但离得远 ④ 只命中一部分 */
        var tier = wholeHit.length ? 1 : (covered && tagged === 0 && near >= 0.7 ? 2 : (covered ? 3 : 4));
  ...
  var DEFAULT_SETTINGS = { suggest: false, sensitivity: "mid", filterNames: true, tooltipDelay: 618 };
  ...
  function buildGenreGrid() {
    ...
    order.forEach(function (pair) {
      ...
      card.type = "button";
      card.innerHTML =
        '<span class="gc-icon">' + genreIcon(g) + "</span>" +
        '<span class="gc-name">' + g + "</span>" +
        '<span class="gc-desc">' + esc(D.GENRE_META[g].desc) + "</span>" +
        '<span class="gc-hint">' + esc(D.GENRE_META[g].hint) + "</span>";
  ...
  function openSearch(genre) {
    ...
    $("#searchPlaceholder").textContent = "在「" + genre + "」里找写过同样意象的句子";
  ...
  function buildTabs() {
    ...
    ["小说", "散文", "戏剧", "诗歌"].forEach(function (g) {
      ...
      b.addEventListener("click", function () {
        ...
        $("#searchPlaceholder").textContent = "在「" + g + "」里找同频的写法";
  ...
  function renderResults() {
    ...
    head.innerHTML =
      ...
      '<span class="rh-meta">排序：恨海情天相关度 → 外国文学 → 现当代 → 中国古代</span>';
  ...
  function resultNode(r, idx) {
    {
      ...
      var lead = allInText ? "这句里同时有：" : "这一段里同时写着：";
      var unitLine = lead + r.hits.map(function (h) {
        ...
        return '<i class="hit' + (how === "text" ? "" : " tagged") + '">' + esc(h) + (how === "text" ? "" : "·场景") + "</i>";
      }).join("") +
